...
*/
FirstStageScene::FirstStageScene(const Scene& _NowScene)
	: mTransitionFlag(false)
{
	...
	mSprite = new Sprite("Assets/BackGround/FirstStage.png");
	...
	for (int i = 0; i < 3; i++)
	{
		if (i <= 1)
		{
			mCheckPointBoard = new CheckpointBoard(Vector3(800.0f, 300.0f, (-81100.0f + i * 25000.0f)), Vector3::sZERO, "Assets/Texture/Board/FirstStageCheckpoint.png", Tag::eCheckpointTag, _NowScene);
		}
		else
		{
			mCheckPointBoard = new CheckpointBoard(Vector3(800.0f, 300.0f, -39700.0f), Vector3::sZERO, "Assets/Texture/Board/FirstStageCheckpoint.png", Tag::eCheckpointTag, _NowScene);
		}
	}

	//ゴールワープ板生成
	mGoalWarpBorard = new GoalWarpBorard(Vector3(800.0f, 350.0f, -7800.0f), Vector3::sZERO, "Assets/Texture/Board/Warp.png", Tag::eOtherTag, _NowScene);
...
*/
SceneBase* FirstStageScene::UpdateScene(const InputState& _KeyState)
{
	//次のシーンに遷移するタイミング
